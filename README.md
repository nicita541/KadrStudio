...
- video-only прокси 960×540 CFR с коротким GOP, проверкой повреждений и ограниченным LRU-кэшем;
...
Требуется .NET 10 SDK. Запустите `build-release.bat`. Сценарий одной командой выполняет restore, Release build с warnings-as-errors, unit/integration tests, self-contained publish и создаёт:
...
Архитектура описана в [ARCHITECTURE.md](ARCHITECTURE.md), предпросмотр — в [docs/PREVIEW_ARCHITECTURE.md](docs/PREVIEW_ARCHITECTURE.md), работа в редакторе — в [docs/USER_GUIDE.md](docs/USER_GUIDE.md).
