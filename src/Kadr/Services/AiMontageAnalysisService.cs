...
{
    public const string PipelineVersion = "gameplay-analysis-v1";
        ...
        CancellationToken cancellationToken)
    {
        ...
        var segments = baseline.ToBuilder();

        if (request.DeepAnalysis && !string.IsNullOrWhiteSpace(request.Model))
        {
            var technicalResult = RestoreTechnicalResult(source, baseline);
            var semantic = await ollama.AnalyzeGameplayAsync(
    ...
    private async Task<ImmutableArray<AnalysisSegment>> GetOrCreateBaselineAsync(
        MediaSource source,
        string fingerprint,
        MediaAnalysisRequest request,
        IProgress<VideoAnalysisProgress> progress,
        CancellationToken cancellationToken)
    {
        ...
        {
            var restored = JsonSerializer.Deserialize<MediaAnalysisManifest>(payload.Span);
            if (restored is not null) return restored.Segments;
        }

        var asset = ToUiAsset(source);
        var pipeline = await orchestrator.AnalyzeAsync(
            new VideoAnalysisRequest(
                asset, 0, source.Duration.TotalSeconds, BuildBaselineQuery()),
            ollamaModel: null,
            progress,
            cancellationToken,
        ...
        foreach (var range in pipeline.Result.Ranges)
        {
            ...
            segments.Add(new AnalysisSegment(
                ...
                range.Confidence,
                [new AnalysisEvidence(MontageEvidenceKind.Technical, range.Description, range.Title)]));
        }
    ...
    private static string BuildBaselineQuery()
        => "Технический анализ сцен, движения, тишины и речи игрового видео.";
